...
using tree_dec_var_t = boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS, var_bag>;

static tree_dec_var_t intersection_treedecomp(cfg_t &cfg, tree_dec_t &treedec) {
  tree_dec_var_t result;
  size_t treewidth = 0;
  size_t intersection_treewidth = 0;
  for (int i = 0; i < boost::num_vertices(treedec); i++) {
    boost::add_vertex(result);
    treewidth = std::max(treewidth, treedec[i].bag.size());
    for (auto v : treedec[i].bag) {
      result[i].bag.insert(cfg[v].alive.begin(), cfg[v].alive.end());
    }
    intersection_treewidth = std::max(intersection_treewidth, result[i].bag.size());
  }
  for (int i = 0; i < boost::num_vertices(treedec); i++) {
    tree_dec_t::adjacency_iterator c, c_end;
    boost::tie(c, c_end) = boost::adjacent_vertices(i, treedec);
    for (; c != c_end; c++) {
      boost::add_edge(i, *c, result);
    }
  }
  treewidth -= 1;
  std::cout << "treewidth = " << treewidth << ", intersection_treewidth = " << intersection_treewidth << ", cfg size = " << boost::num_vertices(cfg) << std::endl;
  return result;
}

using coloring_t = std::vector<std::pair<var_t, int>>;
namespace std {
  template<> struct hash<coloring_t> {
    std::size_t operator()(coloring_t const &c) const {
...
// timeout of 10s
static inline bool is_timeout(bool shouldThrow = true) {
  struct timespec ts;
  timespec_get(&ts, TIME_UTC);
  if (shouldThrow && ts.tv_sec - start_time > 10) {
    throw std::runtime_error("timeout");
  }
  return ts.tv_sec - start_time > 10;
}

using Puzzle_piece = std::tuple<int, int, int>;

struct Puzzle {
  std::vector<int> board;
  std::map<int, int> &global_reg;
  Puzzle(int r, std::map<int, int> &global_reg)
      : board(), global_reg(global_reg) {
    board.resize(r);
    for (int i = 0; i < r; i++) {
      board[i] = 0;
    }
  ...
  bool isSolved(std::vector<Puzzle_piece> pieces) {
    for (int i = 0; i < pieces.size(); i++) {
      if (std::get<2>(pieces[i]) != -1) {
        board[std::get<2>(pieces[i])] =
            board[std::get<2>(pieces[i])] + std::get<1>(pieces[i]);
        continue;
      }
      int j;
      for (j = 0; j < board.size(); j++) {
        is_timeout();
        if (board[j] + std::get<1>(pieces[i]) > 11 ||
            board[j] == std::get<1>(pieces[i])) {
          continue;
        } else {
          board[j] = board[j] + std::get<1>(pieces[i]);
          global_reg[std::get<0>(pieces[i])] = j;
          break;
        }
      }
      if (j >= board.size()) {
    ...
    for (int i = 0; i < board.size(); i++) {
      if (not(board[i] == 0 || board[i] == 1 || board[i] == 10 ||
              board[i] == 11)) {
        return false;
      }
    }
...
struct Puzzle_piece_list {
  std::map<int, Puzzle_piece> pmap;
  std::map<int, int> &global_reg;
  Puzzle_piece_list(boost::graph_traits<cfg_t>::vertex_descriptor in_v,
                    boost::graph_traits<cfg_t>::vertex_descriptor out_v,
                    cfg_t &G, std::map<int, int> &global_reg)
      : pmap(), global_reg(global_reg) {
    auto in = G[in_v];
    auto out = G[out_v];

    for (int i = 0; i < in.alive.size(); i++) {
      if (global_reg.find(in.alive[i]) != global_reg.end()) {
        pmap[in.alive[i]] =
            Puzzle_piece(in.alive[i], 1, global_reg[in.alive[i]]);
        continue;
      }
      pmap[in.alive[i]] = std::make_tuple(in.alive[i], 1, -1);
    }
    for (int i = 0; i < out.alive.size(); i++) {
      if (pmap.find(out.alive[i]) != pmap.end()) {
        pmap[out.alive[i]] =
            std::make_tuple(out.alive[i], 11, std::get<2>(pmap[out.alive[i]]));
        continue;
      }

      if (global_reg.find(out.alive[i]) != global_reg.end()) {
        pmap[out.alive[i]] =
            Puzzle_piece(out.alive[i], 10, global_reg[out.alive[i]]);
        continue;
      }
      pmap[out.alive[i]] = std::make_tuple(out.alive[i], 10, -1);
    }
  }

  std::vector<Puzzle_piece> getlist() {
    std::vector<Puzzle_piece> pl;
    pl.clear();
    for (auto it = pmap.begin(); it != pmap.end(); it++) {
      pl.push_back(it->second);
    }
    sort(pl.begin(), pl.end(), [](Puzzle_piece a, Puzzle_piece b) {
...
inline bool getGlobalReg(int r, cfg_t &cfg) {
  std::map<int, int> global_reg;
  ...
  for (boost::tie(vi, vi_end) = vertices(cfg); vi != vi_end; ++vi) {
    ...
    for (boost::tie(ei, ei_end) = out_edges(*vi, cfg); ei != ei_end; ++ei) {
      boost::graph_traits<cfg_t>::vertex_descriptor t, s;
      t = boost::target(*ei, cfg);
      s = boost::source(*ei, cfg);
      Puzzle_piece_list pl(s, t, cfg, global_reg);
      std::vector<Puzzle_piece> puzzle_list = pl.getlist();

      Puzzle p(r, global_reg);
      if (p.isSolved(puzzle_list) == false) {
        return false;
      }
    }
  }
...
static bool try_coloring_treedec(int w, tree_dec_var_t::vertex_descriptor t,
                                 tree_dec_var_t &T, con_t &con,
                                 phmap::parallel_flat_hash_set<coloring_t> &colorings) {
  bool dead = false;
  std::vector<var_t> vertices(T[t].bag.begin(), T[t].bag.end());
  if (vertices.empty()) {
    colorings.insert(coloring_t());
  } else {
    for (int c = 0; c < w; c++)
      colorings.insert({std::make_pair(vertices.back(), c)});
    vertices.pop_back();
    for (auto v : vertices) {
      std::vector<coloring_t> old_colorings(colorings.begin(), colorings.end());
      phmap::parallel_flat_hash_set<coloring_t> new_colorings;
      for (int c = 0; c < w; c++) {
        for (auto &coloring : old_colorings) {
          if (is_timeout(false))
            break;
          bool possible = true;
          for (auto &p : coloring) {
            if (p.second == c && 
                // live range conflict
                boost::edge(p.first, v, con).second) {
              possible = false;
              break;
            }
          }
          if (possible && !dead) {
            try {
              coloring_t tmp = coloring;
              tmp.push_back(std::make_pair(v, c));
              new_colorings.insert(tmp);
            } catch (std::bad_alloc &) {
              dead = true;
              new_colorings.clear();
              colorings.clear();
  ...
  tree_dec_var_t::adjacency_iterator c, c_end;
  tree_dec_var_t::vertex_descriptor c0, c1;
  boost::tie(c, c_end) = boost::adjacent_vertices(t, T);

  for (; c != c_end; c++) {
    phmap::parallel_flat_hash_set<coloring_t> new_colorings;
    phmap::parallel_flat_hash_set<coloring_t> agreed_colorings;
    if (!try_coloring_treedec(w, *c, T, con, new_colorings))
      return false;
    std::vector<coloring_t> new_colorings_vec(new_colorings.begin(), new_colorings.end());
    for (auto &coloring1 : colorings) {
      for (auto &coloring2 : new_colorings_vec) {
        if (dead || is_timeout(false))
          break;
        try {
          boost::container::flat_map<var_t, int> register_mapping;
          bool agreed = true;
          for (auto &p : coloring1)
            register_mapping[p.first] = p.second;
          for (auto &p : coloring2) {
            if (is_timeout(false))
              break;
            if (!register_mapping.insert(p).second) {
              // variable already added
              if (register_mapping[p.first] != p.second) {
                agreed = false;
                break;
              }
            } else {
              // new variable, check if there is any conflict in register
              // assignment
              for (auto &p2 : coloring1) {
                if (p2.second == p.second && 
                    boost::edge(p.first, p2.first, con).second) {
                  agreed = false;
                  break;
                }
              }
          ...
          if (agreed) {
            agreed_colorings.insert(coloring_t(
                  register_mapping.begin(), register_mapping.end()));
          }
        } catch (std::bad_alloc &) {
          agreed_colorings.clear();
          colorings.clear();
          dead = true;
          break;
        }
    ...
    colorings = agreed_colorings;
  }
  // restrict to vertices
  phmap::parallel_flat_hash_set<coloring_t> new_colorings;
  for (auto &coloring : colorings) {
    is_timeout();
    coloring_t restricted;
    for (auto &p : coloring)
      if (std::find(vertices.begin(), vertices.end(), p.first) != vertices.end())
        restricted.push_back(p);
    new_colorings.insert(restricted);
  }
  colorings = new_colorings;
  return !colorings.empty();
}

static std::vector<var_t> intersection_decomp(cfg_t &cfg) {
  Cfg<cfg_t> cfg_(cfg);
  std::vector<boost::container::flat_set<var_t>> decomp(
      cfg_.GetPathDec().size());
  int pathwidth = 0;
  for (int i = 0; i < decomp.size(); i++) {
    if (cfg_.GetPathDec()[i].size() > pathwidth)
      pathwidth = cfg_.GetPathDec()[i].size();
    for (auto v : cfg_.GetPathDec()[i]) {
      decomp[i].insert(cfg[v].alive.begin(), cfg[v].alive.end());
    }
  }
  pathwidth -= 1;
  // nicify
  int intersection_pathwidth = 0;
  boost::container::flat_set<var_t> current_set;
  std::vector<var_t> tmp;
  std::vector<var_t> nice_decomp;
  for (auto &bag : decomp) {
    std::set_difference(current_set.begin(), current_set.end(),
                        bag.begin(), bag.end(), std::back_inserter(tmp));
    nice_decomp.insert(nice_decomp.end(), tmp.begin(), tmp.end());
    for (auto &v : tmp)
      current_set.erase(v);
    tmp.clear();
    std::set_difference(bag.begin(), bag.end(), current_set.begin(),
                        current_set.end(), std::back_inserter(tmp));
    nice_decomp.insert(nice_decomp.end(), tmp.begin(), tmp.end());
    current_set.insert(tmp.begin(), tmp.end());
    if (current_set.size() > intersection_pathwidth)
      intersection_pathwidth = current_set.size();
    tmp.clear();
  }
  nice_decomp.insert(nice_decomp.end(), current_set.begin(), current_set.end());
  int w_min = intersection_pathwidth/(pathwidth + 1);
  std::cout << "pathwidth = " << pathwidth << ", intersection_pathwidth = " << intersection_pathwidth << ", cfg size = " << boost::num_vertices(cfg) << std::endl;
  return nice_decomp;
}

static bool try_coloring(int w, std::vector<var_t> &decomp, con_t &intersection_graph) {
  std::vector<coloring_t> colorings;
  std::vector<var_t> alive;
  for (auto v : decomp) {
    phmap::parallel_flat_hash_set<coloring_t> new_colorings;
    auto iter  = std::find(alive.begin(), alive.end(), v);
    bool dead = false;
    if (iter != alive.end()) {
      // forget node
      alive.erase(iter);
      for (coloring_t coloring : colorings) {
        if (is_timeout(false))
          continue;
        for (auto elem = coloring.begin(); elem != coloring.end(); elem++) {
          if (std::get<0>(*elem) == v) {
            coloring.erase(elem);
            break;
          }
        ...
        try {
          new_colorings.insert(std::move(coloring));
      ...
      for (const auto &ci : colorings) {
        ...
        try {
          ...
          for (int i = 0; i < w; i++) {
            bool possible = true;
            bool colorOccurred = false;
            for (auto &p : ci) {
              if (std::get<1>(p) == i)
                colorOccurred = true;
              if (std::get<1>(p) == i && 
                  // live range conflict
                  boost::edge(std::get<0>(p), v, intersection_graph).second) {
                possible = false;
                break;
              }
            }
            if (!colorOccurred) {
              // skip if they are the same modulo register renaming
              if (newcolor)
                continue;
              newcolor = true;
            }
            if (possible) {
              coloring_t coloring = ci;
              coloring.push_back(std::make_pair(v, i));
              new_colorings.insert(coloring);
            }
          }
        } catch (const std::bad_alloc &) {
          new_colorings.clear();
      ...
      if (colorings.empty() && w > 0) {
        new_colorings.insert({std::make_pair(v, 0)});
      }
    }
    is_timeout();
    if (dead) {
    ...
    try {
      colorings = std::vector<coloring_t>(
          std::make_move_iterator(new_colorings.begin()),
          std::make_move_iterator(new_colorings.end()));
    } catch (const std::bad_alloc&) {
      std::cout << "OOM when w = " << w << std::endl;
      throw std::runtime_error("oom");
    }
